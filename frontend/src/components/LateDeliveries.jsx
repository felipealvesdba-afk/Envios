...
function FiltersBar({ q, setQ, forma, setForma, situacao, setSituacao, options }) {
  return (
...
function LateTable({ rows, onOpenTicket }) {
  return (
...
function TableContainer({ loading, rows, onOpenTicket }) {
  if (loading) {
    return (
  ...
  return <LateTable rows={rows} onOpenTicket={onOpenTicket} />;
}
...
export default function LateDeliveries({ onOpenTicket, refreshKey }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      console.error("Erro ao carregar entregas atrasadas", err);
      setRows([]);
      setTotal(0);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const t = setTimeout(load, SEARCH_DEBOUNCE_MS);
  ...
  return (
    <div className="space-y-6" data-testid="late-view">
      ...
      <div className="border border-zinc-200 bg-white">
        <TableContainer loading={loading} rows={rows} onOpenTicket={onOpenTicket} />
      </div>
    </div>
  );
}
